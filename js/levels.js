function randomizeLevels(rom) {
    ...
    const normalOffsets = [0x3C218, 0x3C23B, 0x3C239, 0x3C23C, 0x3C240,
    ...
    const secretOffsets = [0x3C23A, 0x3C241, 0x3C242, 0x3C260, 0x3C21D,
    0x3C254, 0x3C24A];
    //level offset and zone clear byte
    const bossSetOne = [[0x3C238, 0x05], [0x3C243, 0x09], [0x3C24D, 0x0E],
	...
	normalOffsets.forEach((offset, index) => {
		rom[offset] = normalBytes[index];
	});
    shuffle(secretBytes);
	secretOffsets.forEach((offset, index) => {
		rom[offset] = secretBytes[index];
	});
    shuffle(bossSetTwo);
