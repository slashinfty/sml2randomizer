...
function randomizeBossHealth(rom) {
	...
	healthSetOne.forEach(offset => {
		rom[offset] = (0x02 * (prng.nextInt(2) + 2));
	});
	healthSetTwo.forEach(offset => {
		rom[offset] = (0x02 * (prng.nextInt(4) + 3));
	});
	healthSetThree.forEach(offset => {
		rom[offset] = (0x02 * (prng.nextInt(3) + 3));
	});
}
