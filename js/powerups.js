function randomizePowerups(rom) {
    ...
    const castle = [0x1B, 0x1C, 0x1D, 0x1F];
    ...
    for (let i = 0xE077; i < 0xEBB5; i += 3) {
        ...
        if (rom[i] == 0xFF) {
            ...
            if ((i >= 0xE0BD) && (i < 0xE123)) {
                if (s != 0x1F) {
                    let n = sprite.insert(rom[i], rom[i + 1], free[prng.nextInt(free.length - 1)]);
                    sprite.copy(n, rom, i);
                }
            //only include 0x1F if it is the original
            } else if (s == 0x1F) {
                let n = sprite.insert(rom[i], rom[i + 1], free[prng.nextInt(free.length)]);
                sprite.copy(n, rom, i);
            } else {
                let n = sprite.insert(rom[i], rom[i + 1], free[prng.nextInt(free.length - 1)]);
                sprite.copy(n, rom, i);
            ...
            const prob = [0.2, 0.05, 0.15, 0.05, 0.05, 0.5];
            let p = beastMode ? sprite.bias(block, prob) : block[prng.nextInt(block.length)];
    ...
    rom[0x606AF] = 0x28;
    rom[0x60925] = 0xCA;
    const conveyorPowerups = [0x00, 0x01, 0x02, 0x03, 0x04];
    const conveyorProbs = [0.25, 0.15, 0.15, 0.4, 0.05];
    ...
    const wirePowerups = [0x01, 0x02, 0x03, 0x04];
